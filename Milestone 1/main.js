...
const recapDataEl = document.querySelector('.recapData')
const priceEl = document.querySelector('.price')

formEl.addEventListener('submit', function (e){
    e.preventDefault()
    recapDataEl.innerHTML = `${inputKmEl.value}Km, ${inputAgeEl.value} Your Age`
    ...
    if (Number(inputAgeEl.value) < 18) {
        //console.log(underAgeSale.toFixed(2));
        priceEl.innerHTML = `${underAgeSale.toFixed(2)}&euro;`;
    } else if (Number(inputAgeEl.value) >= 65) {
        //console.log(overAgeSale.toFixed(2));
        priceEl.innerHTML = `${overAgeSale.toFixed(2)}&euro;`;
    } else {
        priceEl.innerHTML = `${priceTicket}&euro;`;
        //console.log(priceTicket);
    }
})
